...
# Phase 16: Cognitive architecture upgrade genes
PHASE16_OFFSET = EMERGENCE_OFFSET + EMERGENCE_COUNT  # 87
PHASE16_COUNT = 4
...
def random_genome(cfg: Config, max_nn_params: int, rng: np.random.Generator) -> np.ndarray:
    ...
    genes[ARCH_OFFSET + 1] = rng.uniform(16, 128)    # Phase 16: wider layer1
    genes[ARCH_OFFSET + 2] = rng.uniform(12, 64)     # Phase 16: wider layer2
    genes[ARCH_OFFSET + 3] = rng.uniform(8, 48)      # Phase 16: wider layer3
    genes[ARCH_OFFSET + 4] = rng.uniform(8, 32)      # Phase 16: wider layer4
    genes[ARCH_OFFSET + 5] = rng.uniform(16, 128)    # Phase 16: wider GRU

    for i in range(MORPH_COUNT):
        genes[MORPH_OFFSET + i] = rng.uniform(0.0, 0.6)

    genes[CONCEPT_OFFSET + 0] = rng.uniform(8.0, 48.0)    # Phase 16: wider bottleneck
    genes[CONCEPT_OFFSET + 1] = rng.uniform(1.0, 5.0)    # Phase 16: more think steps
    ...
    # Phase 16: Cognitive architecture genes
...
def get_trait(genome: np.ndarray, name: str) -> float:
    ...
    elif name in PHASE16_LOCI:
        idx, lo, hi = PHASE16_LOCI[name]
    else:
...
def clamp_genome(genome: np.ndarray):
    ...
    for i in range(1, 5):
        genome[ARCH_OFFSET + i] = np.clip(genome[ARCH_OFFSET + i], 8, 1024)  # Phase 16: up from 256
    genome[ARCH_OFFSET + 5] = np.clip(genome[ARCH_OFFSET + 5], 16, 768)  # Phase 16: up from 128
    for i in range(MORPH_COUNT):
    ...
    for name, (idx, lo, hi) in EMERGENCE_LOCI.items():
        genome[idx] = np.clip(genome[idx], lo, hi)
    for name, (idx, lo, hi) in PHASE16_LOCI.items():
        genome[idx] = np.clip(genome[idx], lo, hi)
