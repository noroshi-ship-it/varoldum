...
# Physics constants only — physical limits, not decisions.
MAX_SOCIAL_RANGE = 4.0       # max distance for social force transmission
...
RETALIATION_FACTOR = 0.6     # defender always retaliates (physics: Newton's 3rd law)
...
class SocialSystem:
    ...
    def resolve_social(self, agents: list, actions: list[dict], tick: int,
        ...
        """
        Pure physics social resolution.
        Brain outputs (social_give, social_take, signal) are the ONLY drivers.
        Trust, kinship, reciprocity are NOT in any formula —
        the brain sees them as inputs and decides what to output.
        """
        ...
        for agent, action in zip(agents, actions):
            ...
            give_intent = action.get("social_give", 0.0)
            take_intent = action.get("social_take", 0.0)
            ...
            for other, dist in nearby:
                # Physics: proximity falloff
                proximity = 1.0 - dist / MAX_SOCIAL_RANGE

                # ============================================================
                # ENERGY SHARING
                # Force = brain's give_intent * proximity * available_energy
                # NO trust multiplier, NO kinship bonus — brain decides.
                # ============================================================
                give_force = max(0.0, give_intent) * proximity
                available = max(0.0, agent.body.energy - 0.2)  # physics: survival reserve
                ...
                if share > 0.001:
                    ...
                    # Physics consequence: trust/bond update proportional to transfer
                ...
                # ============================================================
                # AGGRESSION
                # Force = brain's take_intent * proximity * size_advantage
                # NO trust suppression — brain sees trust as input, decides.
                # ============================================================
                size_ratio = agent.body.size / max(0.01, other.body.size)
                take_force = max(0.0, take_intent) * proximity * min(size_ratio, 2.0)
                ...
                if take_force > 0.01:
                    ...
                    # Physics: retaliation damage (Newton's 3rd law)
                    ...
                    # Physics consequence: trust destruction from aggression
                    if hasattr(other, 'trust_memory'):
                ...
                # ============================================================
                # MINERAL TRADE
                # Force = signal * proximity — brain controls signal.
                # ============================================================
                ...
                # ============================================================
                # INVENTORY TRADE
                # Force = signal * proximity — brain + genome (trade_willingness)
                # ============================================================
                if (substance_props is not None and
                    ...
                    inv_trade_force = signal * trade_will * proximity
